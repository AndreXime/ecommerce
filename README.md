...
Monorepo com **API REST** (`api/`) e **loja web** (`web/`) integradas. O backend cobre catálogo, carrinho, wishlist, pedidos e perfil com autenticação JWT em cookies. O frontend é **Lojas Ximenes**: vitrine Astro de loja geral, carrinho e checkout contra a API e painel admin, sempre usando o backend configurado em `PUBLIC_API_URL`.
